for file in $1/*; do
  ...
  then
    ...
    minizinc -m MCP_CP_norandom.mzn -d $file --time-limit 300000 --solver chuffed -D mzn_ignore_symmetry_breaking_constraints=false --output-time --output-objective --output-mode json --json-stream --search-complete-msg "optimal" 2> /dev/null | head -n -2 >> .temp/temp_$num.out
